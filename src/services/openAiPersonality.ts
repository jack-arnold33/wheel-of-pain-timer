...
interface OpenAiPersonalityEnvironment {
    ...
    try {
      response = await environment.fetch(OPENAI_RESPONSES_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          instructions:
            'Create concise workout sayings from only the supplied information. Do not invent personal facts. Return data matching the supplied JSON schema.',
          input: buildPersonalityBrief(draft, participants, crewProfile),
          text: {
            format: {
              type: 'json_schema',
